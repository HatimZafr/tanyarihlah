...
// Menyimpan state pengguna dalam file
function saveUserState($chatId, $state) {
    $filename = 'user_states.json';
    $states = [];
    
    if (file_exists($filename)) {
        $states = json_decode(file_get_contents($filename), true);
    }
    
    $states[$chatId] = $state;
    file_put_contents($filename, json_encode($states));
}

// Mengambil state pengguna dari file
